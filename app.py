...
def gerenciar_estoque():
    return render_template("estoque/painel.html", secao="estoque", breadcrumbs=breadcrumbs_padrao(("Controle de Estoque", None)))
...
def api_produtos():
    ...
    if request.method == "POST":
        dados = request.json
        try:
            quantidade_atual = int(dados.get("quantidade_atual", 0))
            quantidade_minima = int(dados.get("quantidade_minima", 0))
            valor_compra = float(dados.get("valor_compra", 0))
            campos_obrigatorios = ("nome", "categoria", "fornecedor", "data_validade", "unidade_medida")
            ...
            if quantidade_atual < 0 or quantidade_minima < 0 or valor_compra < 0:
                raise ValueError("Quantidade e valor não podem ser negativos.")
            connection.execute(
                "INSERT INTO produtos (nome, categoria, fornecedor, quantidade_atual, quantidade_minima, data_validade, valor_compra, unidade_medida) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (dados.get("nome"), dados.get("categoria"), dados.get("fornecedor"), quantidade_atual, quantidade_minima, dados.get("data_validade"), valor_compra, dados.get("unidade_medida"))
            )
            connection.commit()
            return jsonify({"sucesso": True, "mensagem": "Produto cadastrado com sucesso!"}), 201
        except Exception as e:
            return jsonify({"sucesso": False, "mensagem": str(e)}), 400
        finally:
            connection.close()
    produtos = connection.execute("SELECT * FROM produtos ORDER BY nome ASC").fetchall()
    connection.close()
    return jsonify([serializar_row(p) for p in produtos])
...
def atualizar_produto(produto_id):
    ...
    try:
        ...
        if quantidade_atual < 0 or quantidade_minima < 0 or valor_compra < 0:
            raise ValueError("Quantidade e valor não podem ser negativos.")
        produto = connection.execute("SELECT id FROM produtos WHERE id = ?", (produto_id,)).fetchone()
        if not produto:
            return jsonify({"sucesso": False, "mensagem": "Produto não encontrado."}), 404
        connection.execute(
            """
            UPDATE produtos
            SET nome = ?, categoria = ?, fornecedor = ?, quantidade_atual = ?,
                quantidade_minima = ?, data_validade = ?, valor_compra = ?, unidade_medida = ?
            WHERE id = ?
            """,
            (
                dados.get("nome"), dados.get("categoria"), dados.get("fornecedor"),
                quantidade_atual, quantidade_minima, dados.get("data_validade"), valor_compra,
                dados.get("unidade_medida"), produto_id,
            ),
        )
        connection.commit()
...
@app.route("/api/entradas", methods=["GET", "POST"])
@login_obrigatorio
def api_entradas():
    connection = get_db_connection()
    if request.method == "GET":
        entradas = connection.execute(
            """
            SELECT entradas_estoque.*, produtos.nome AS produto_nome
            FROM entradas_estoque
            JOIN produtos ON produtos.id = entradas_estoque.produto_id
            ORDER BY entradas_estoque.id DESC
            """
        ).fetchall()
        connection.close()
        return jsonify([serializar_row(entrada) for entrada in entradas])
    ...
    try:
        ...
        validade_lote = dados.get("validade_lote")
        if quantidade <= 0 or valor_compra < 0 or not validade_lote:
            raise ValueError("Informe quantidade, validade e valor de compra válidos.")
        produto = connection.execute("SELECT id FROM produtos WHERE id = ?", (produto_id,)).fetchone()
        if not produto:
            raise ValueError("Produto não encontrado.")
        data_registro = datetime.now().strftime("%Y-%m-%dT%H:%M:%S")
        connection.execute(
            "INSERT INTO entradas_estoque (produto_id, quantidade, validade_lote, valor_compra, data_registro) VALUES (?, ?, ?, ?, ?)",
            (produto_id, quantidade, validade_lote, valor_compra, data_registro),
        )
        connection.execute(
            "UPDATE produtos SET quantidade_atual = quantidade_atual + ?, data_validade = ?, valor_compra = ? WHERE id = ?",
            (quantidade, validade_lote, valor_compra, produto_id),
        )
        connection.commit()
        return jsonify({"sucesso": True, "mensagem": "Entrada registrada com sucesso!"}), 201
...
def api_saidas():
    ...
    try:
        produto_id = int(dados.get("produto_id"))
        quantidade_saida = int(dados.get("quantidade"))
        motivo = dados.get("motivo")
        cliente_tutor = dados.get("cliente_tutor")
        if quantidade_saida <= 0 or motivo not in ("avaria", "validade", "venda") or not cliente_tutor:
            raise ValueError("Informe produto, quantidade, cliente/tutor e um motivo válido.")
        produto = connection.execute("SELECT quantidade_atual FROM produtos WHERE id = ?", (produto_id,)).fetchone()
        if not produto or produto["quantidade_atual"] < quantidade_saida:
            return jsonify({"sucesso": False, "mensagem": "Estoque insuficiente!"}), 400
        connection.execute("UPDATE produtos SET quantidade_atual = quantidade_atual - ? WHERE id = ?", (quantidade_saida, produto_id))
        connection.execute(
            "INSERT INTO saidas_estoque (produto_id, quantidade, cliente_tutor, motivo, data_registro) VALUES (?, ?, ?, ?, ?)",
            (produto_id, quantidade_saida, cliente_tutor, motivo, datetime.now().strftime("%Y-%m-%dT%H:%M:%S"))
        )
        connection.commit()
