...
class UniVetAppTests(unittest.TestCase):
    ...
    def test_fluxo_completo_de_estoque(self):
        self._login("admin", "123456")
        produto = {
        ...
        resposta = self.client.post("/api/produtos", json=produto)
        self.assertEqual(resposta.status_code, 201)
        conexao = self._conexao()
        produto_id = conexao.execute(
            "SELECT id FROM produtos WHERE nome = ?", (produto["nome"],)
        ).fetchone()["id"]
        conexao.close()
        ...
        resposta = self.client.put(
            ...
            json={**produto, "quantidade_atual": 99},
        )
        self.assertEqual(resposta.status_code, 200)

        conexao = self._conexao()
        produto_atualizado = conexao.execute(
            "SELECT quantidade_atual, data_validade, valor_compra FROM produtos WHERE id = ?",
            (produto_id,),
        ).fetchone()
        entrada = conexao.execute(
            "SELECT quantidade, validade_lote, valor_compra FROM entradas_estoque WHERE produto_id = ?",
            (produto_id,),
        ).fetchone()
        saida = conexao.execute(
            "SELECT quantidade, cliente_tutor, motivo FROM saidas_estoque WHERE produto_id = ?",
            (produto_id,),
        ).fetchone()
        conexao.close()
        self.assertEqual(tuple(entrada), (5, "2028-01-31", 20.0))
        self.assertEqual(tuple(saida), (2, "Tutor Teste", "venda"))
        self.assertEqual(tuple(produto_atualizado), (99, "2027-12-31", 18.5))
